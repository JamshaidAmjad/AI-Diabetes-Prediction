...
def cross_validate_model(
    ...
    """k-fold cross-validation on the combined train+val set. Returns mean/std and per-fold scores."""
    ...
    print(f"  CV {scoring}: {result['mean']:.4f} ± {result['std']:.4f}  (folds: {result['fold_scores']})")
    return result
...
def tune_random_forest(
    ...
    """GridSearchCV over key RF hyperparameters. Returns best estimator."""
    param_grid = {
    ...
    gs = GridSearchCV(rf, param_grid, cv=cv, scoring="f1", n_jobs=-1, verbose=0)
...
def main():
    ...
    csv_path = Path(config["data"]["raw_dir"]) / config["data"]["filename"]
    data = build_dataset(
    ...
    # Combine train+val for cross-validation (test set remains untouched)
    ...
    cv_folds = config.get("cross_validation", {}).get("cv_folds", 5)
    cv_scoring = config.get("cross_validation", {}).get("scoring", "f1")
    cv_results = {}
    ...
    if model_type in ("logistic_regression", "all"):
        ...
        print("=" * 50)
        lr_model = train_sklearn_model(build_logistic_regression(seed), X_train, y_train)
        print(" Cross-validation (train+val):")
        cv_results["logistic_regression"] = cross_validate_model(
            build_logistic_regression(seed), X_trainval, y_trainval,
    ...
    if cv_results:
        cv_path = Path(reports_dir) / "cv_results.json"
